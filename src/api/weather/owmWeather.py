    ...
    def getWeather(self):
        ...
        try:
            ...
            if wx_cache is None:
                ...
                url = f"http://api.openweathermap.org/data/2.5/weather?lat={lat}&lon={lon}&units={self.data.config.weather_units}&appid={self.apikey}"
        ...
        if not self.network_issues:
            ...
            wx_code = wx.get("weather")[0].get("id")
            wx_icon = self.getWeatherIcon(wx_code)

            wx_summary = wx.get("weather")[0].get("description")
            wx_wind = wx.get("wind")
            owm_windspeed = wx_wind['speed']
            owm_windgust = wx_wind.get('gust', 0)
            ...
            # Temperature data
            owm_temp = wx.get("main")['temp']
            owm_app_temp = wx.get("main")['feels_like']
            ...
            wx_humidity = str(wx.get('main')['humidity']) + "%"

            # Other weather data
            wx_dewpoint = str(round(dew_point(float(owm_temp), wx.get('main')['humidity']), 1)) + self.data.wx_units[0]
            wx_pressure = str(wx.get('main')['pressure']) + " " + self.data.wx_units[4]
